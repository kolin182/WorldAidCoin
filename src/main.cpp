...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // WorldAidCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "WorldAidCoin Signed Message:\n";
const string CHARITY_ADDRESS = "Vknc7XBf9jgv2HULr5RYUNEZencC8u8axq";
...
{
    // WorldAidCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // WorldAidCoin: 840k blocks in ~4 years
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // WorldAidCoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // WorldAidCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // WorldAidCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2116) { DiffMode = 2; } // WorldAidCoin, 100 blocks after first legacy diff adjustment
        }
        else {         
        	if (pindexLast->nHeight+1 >= 26754) { DiffMode = 2; }  //WorldAidCoin, 5 days after 27/01/2014 12:00 UTC
        }
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    int64 genValue = GetBlockValue(pindex->nHeight, nFees);


    if (vtx[0].GetValueOut() > genValue)
        return state.DoS(100, error("ConnectBlock() : coinbase pays too much (actual=%"PRI64d" vs limit=%"PRI64d")", vtx[0].GetValueOut(), genValue));

    genValue /= 2;


	if (pindex->nHeight > 90000)
    {
        if (vtx[0].vout[0].nValue > genValue)
            return state.DoS(100, error("ConnectBlock() : miner gen pays too much (actual=%"PRI64d" vs limit=%"PRI64d")", vtx[0].GetValueOut(), genValue));

        if (vtx[0].vout[1].nValue > genValue)
            return state.DoS(100, error("ConnectBlock() : donation pays too much (actual=%"PRI64d" vs limit=%"PRI64d")", vtx[0].GetValueOut(), genValue));
    
    

        CBitcoinAddress charityDestinationAddress(CHARITY_ADDRESS);
        CScript charityDestination = vtx[0].vout[1].scriptPubKey;
        charityDestination.SetDestination(charityDestinationAddress.Get());

    	//charityDestination << OP_DUP << OP_HASH160 << ParseHex(CHARITY_ADDRESS) << OP_EQUALVERIFY << OP_CHECKSIG;

    
    
        if (vtx[0].vout[1].scriptPubKey != charityDestination)
            return state.DoS(100, error("ConnectBlock() : coinbase does not pay to the charity in the second output)"));
    }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // WorldAidCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // WorldAidCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // WorldAidCoin: increase each by adding 1 to WorldAidCoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    CBitcoinAddress myaddress(CHARITY_ADDRESS);
    std::string address_string = myaddress.ToString();
    CScript scriptPubKeyInCopy = scriptPubKeyIn;
    scriptPubKeyInCopy.SetDestination(myaddress.Get());
    ...
    txNew.vout[1].scriptPubKey = scriptPubKeyInCopy;
    ...
    int64 nFees = 0;
    {
        ...
        int64 newValue = GetBlockValue(pindexPrev->nHeight+1, nFees) / 2;
        
        //pblock->vtx[0].vout[0].nValue = GetBlockValue(pindexPrev->nHeight+1, nFees);
        
        pblock->vtx[0].vout[0].nValue = newValue;
        pblock->vtx[0].vout[1].nValue = newValue;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WorldAidCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WorldAidCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WorldAidCoinMiner(CWallet *pwallet)
{
    printf("WorldAidCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("WorldAidCoin-miner");
    ...
    try { loop {
        ...
        printf("Running WorldAidCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WorldAidCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WorldAidCoinMiner, pwallet));
}
